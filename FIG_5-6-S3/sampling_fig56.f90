...
module GLOB 
  ...
  real(kind=kind(0.d0))::stochx(nv,nr),stochxt(nr,nv),stoch(nv+nc,nr),stocht(nr,nv+nc)
...
!!! SAMPLING PROCESS
     kcats(nc+1:nr)=(/1D6,1D0,1D0,1D6,1D0/);
     do isamp=1,nsamp
1       xxs=0.01D0;
        continue
        call random_number(kcat(nc+1:nr));
        kcat(nc+1:nr)=10**(8.*kcat(nc+1:nr)-4); !10^-3 to 10^5

        ! Reshape kinetic space to optimize exploration of flux solutions
