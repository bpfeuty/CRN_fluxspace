...
module GLOB
  implicit none
  integer,parameter::nv=8,nr=12,nm=1,npop=1000,nc=2,kn=1
...
program principal 
  ...
  !! Compute thermodynamic variables versus Tsig over a subsample npop=10^3 (FIG.2C)
  open(33,file='fig2C-mub.dat'); !exchanged species only
  open(34,file='fig2C-mu.dat'); !all species
  open(38,file='fig2C-reac.dat'); !all species
  open(37,file='fig2C-reacb.dat'); !exchange species only
  open(39,file='fig2C-mean.dat'); 
...
9    stoch=0;
     ...
     !! exclude the case of no emergent cycle (two broken conservation law)
     ...
     call random_number(mu0); mu0=12D0*(mu0-0.5);
